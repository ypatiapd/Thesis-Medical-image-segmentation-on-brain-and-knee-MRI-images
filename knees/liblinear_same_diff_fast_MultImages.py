...
iterations =1 
classes=[1, 2, 3,4]  
#imgs = ['9040944','9017909','9019287','9023193', '9033937','9034644','9036287','9036770','9036948','9011115','9041946','9047539','9052335','9069761','9073948','9089627','9090290','9093622','9080864']

#imgs=['9033937','9011115','9017909','9019287','9023193','9040944','9033937','9034644','9036287','9036770','9036948']
imgs = ['9011115']#,'9036287' ,'9073948','9090290',  '9069761'  ]
num = 4

threshold = 25 # a fixed brightness boundary to measure the similarity only for the brightest parts of the image 
...
#image1 = sitk.ReadImage('C:/Users/ypatia/diplomatiki/norm_imgs/norm09.hdr')
for k in range(0,len(imgs)):
    target_image = imgs[0]
    '''target = sitk.ReadImage(target_image)
    image1_array = sitk.GetArrayFromImage(target)
    image1_array[image1_array < threshold] = 0


    #filename = 'Dmfnofe'+str(imgs[k])+'.txt'
    # edw na orizw to filename 
    images= list()
    dists = list()
    for ite in imgs:      
         
        dists = list() # IZ THIS RIGHT
        # Read in the second image
        #image2 = sitk.ReadImage('C:/Users/ypatia/diplomatiki/norm_imgs/norm'+ite+'.hdr')
        image2 = sitk.ReadImage('C:/Users/jaime/Desktop/registered/'+str(imgs[k])+'/mri.hdr')
    
        image2_array = sitk.GetArrayFromImage(image2)
        image2_array[image2_array < threshold] = 0
    ...
    print(sorted_list)
    first_seven = sorted_list[:7]
    print(first_seven)
    file_codes = [lst[1] for lst in sorted_list[:num]] # keep the 7 more similar, first is the target - the 6 next are the annotators' sets
    print(file_codes)
    
    #filenames = ['mhrn9034644.txt','mhrn9023193.txt','mhrn9033937.txt','mhrn9036287.txt','mhrn9036770.txt']#,'mhrn9036948.txt','mhrn9040944.txt','mhrn9041946.txt','mhrn9047539.txt','mhrn9052335.txt','mhrn9069761.txt']                                                                
    #filenames = ['mh5rn269011115.txt','mh5rn269017909.txt','mh5rn269019287.txt','mh5rn269023193.txt', 'mh5rn269033937.txt','mh5rn269034644.txt']#,'mh5rn9036287.txt','mh5rn9036770.txt','mh5rn9036948.txt','mh5rn9040944.txt','mh5rn9041946.txt','mh5rn9047539.txt','mh5rn9052335.txt','mh5rn9069761.txt']
    #filenames = ['mh5rn9011115.txt','mh5rn9017909.txt','mh5rn9019287.txt','mh5rn9023193.txt', 'mh5rn9033937.txt','mh5rn9034644.txt']#,'mh5rn9036287.txt','mh5rn9036770.txt','mh5rn9036948.txt','mh5rn9040944.txt','mh5rn9041946.txt','mh5rn9047539.txt','mh5rn9052335.txt','mh5rn9069761.txt']
    #filenames = ['Dmfnofe9019287.txt','Dmfnofe9034644.txt','Dmfnofe9023193.txt','Dmfnofe9033937.txt','Dmfnofe9036287.txt','Dmfnofe9036770.txt','Dmfnofe9036948.txt','Dmfnofe9011115.txt']  # 'Dmfnofe9040944.txt'
    
    # edw na orizw ta filenames
        
    filename = list()
    filename = 'SYFOGLCRLM'+str(file_codes[0])+'.txt' # urgent change
   
    print(file_codes[0])
    filenames = []
    for d in range(1,len(file_codes)):
        filenames.append('SYFOGLCRLM'+str(file_codes[d])+'.txt')'''
    filenames=list()
    ...
    '''
    Y20, X20 = svm_read_problem(filename)
    ...
    # iterate over the values in the dictionary
        for value in d.values():
            ...
            if math.isnan(value):
                print("Found NaN value in dictionary:", d)
                break
    '''
    ...
    for i in range(0,B):
        print('reading file')
        '''Y10, X10 = svm_read_problem(filenames[i])
