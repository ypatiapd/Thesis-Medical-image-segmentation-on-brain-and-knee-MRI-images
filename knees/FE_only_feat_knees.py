...
for ite in iids:  
    ...
    a.SetRegionOfInterest([15,74,101,116,203,157])
    ...
    for z in range(0,len(featlist)):
        ...
        if z<len(featlist)-1: #z<2
            ...
            df_im = pd.DataFrame(dataset.loc[:,0:12].values,columns = [str(0), str(1) , str(2),str(3), str(4) , str(5),str(6),str(7),str(8),str(9),str(10),str(11),str(12)]) #stin teleutaia epanalipsi pairnw tis treis prwtes steiles pou einai oi klaseis, to brightness kai lbp 
            
            df2 = dataset.loc[:,13:len(featlist[z])+13].values #pairnw kai tis upoloipes pou einai ta xaraktiristika gia pca
