...
#a.SetRegionOfInterest([90,102,60,30,34,30])
...
for z in range(0,2):
    ...
    if z<1: #z<2
        ...
        df = StandardScaler().fit_transform(df3)
        ...
        for j in range(1,len(dataset)-1):
            df1 = pd.DataFrame(dataset[j]).T
            df2 = df2.append(copy.copy(df1), ignore_index=True)   
        df_im = pd.DataFrame(df2.loc[:,0:2].values,columns = [str(0), str(1) , str(2)])
        df3 = df2.loc[df_im["0"]!=0, 3:3+len(featlist[z])].values
