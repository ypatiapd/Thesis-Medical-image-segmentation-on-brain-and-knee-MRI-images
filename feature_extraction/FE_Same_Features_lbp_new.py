...
imageName='C:/Users/ypatia/diplomatiki/disc1/OAS1_0005_MR1/PROCESSED/MPRAGE/SUBJ_111/OAS1_0005_MR1_mpr_n4_anon_sbj_111.hdr'
maskName='C:/Users/ypatia/diplomatiki/disc1/OAS1_0005_MR1/FSL_SEG/OAS1_0005_MR1_mpr_n4_anon_111_t88_masked_gfc_fseg.hdr'
...
d=next(L) #kyrtosis
kyrtosis= d[0]
lbp_arr = sitk.GetArrayFromImage(level2)
...
for z in range(0,2):
    ...
    if z<1: #z<2
        ...
        for j in range(1,len(dataset)-1):
            ...
            df2 = df2.append(copy.copy(df1), ignore_index=True)'''
            
        #df_im = pd.DataFrame(df2.loc[:,0].values,columns = [str(0)])
        #df3 = df2.loc[df_im["0"]!=0,1: 1+len(featlist[z])].values
        ...
        principalDf = pd.DataFrame(data = principalComponents
                     , columns = [str(7*z+3), str(7*z+4), str(7*z+5), str(7*z+6),str(7*z+7),str(7*z+8),str(7*z+9)])#,str(10*z+9),str(10*z+10),str(10*z+11)])
        ...
        df_im = pd.DataFrame(dataset.loc[:,0:2].values,columns = [str(0), str(1) , str(2)]) #stin teleutaia epanalipsi pairnw tis treis prwtes steiles pou einai oi klaseis, to brightness kai lbp 
        
        df2 = dataset.loc[:,3:len(featlist[z])+3].values #pairnw kai tis upoloipes pou einai ta xaraktiristika gia pca
        df = StandardScaler().fit_transform(df2)
        
        pca = PCA(n_components=7)
        principalComponents = pca.fit_transform(df)
        principalDf = pd.DataFrame(data = principalComponents
                     , columns = [str(7*z+3), str(7*z+4), str(7*z+5), str(7*z+6),str(7*z+7),str(7*z+8),str(7*z+9)])         
        
        pdf = pd.concat([pdf,principalDf], axis=1)
        #df_im[:,1] = StandardScaler().fit_transform(df_im[:,1])
        
        #df_im = pd.DataFrame(df_im.loc[df_im["0"]!=0,:].values,columns = [str(0), str(1) , str(2)])
