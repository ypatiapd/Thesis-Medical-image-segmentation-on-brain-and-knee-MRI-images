    ...
    for z in range(0,2):
        ...
        if z<1: #z<2
            ...
            principalDf = pd.DataFrame(data = principalComponents
                         , columns = [str(7*z+6), str(7*z+7), str(7*z+8), str(7*z+9),str(7*z+10),str(7*z+11),str(7*z+12)])#,str(10*z+9),str(10*z+10),str(10*z+11)])
            ...
            df_im = pd.DataFrame(dataset.loc[:,0:5].values,columns = [str(0), str(1) , str(2),str(3), str(4) , str(5)]) #stin teleutaia epanalipsi pairnw tis treis prwtes steiles pou einai oi klaseis, to brightness kai lbp 
            
            df2 = dataset.loc[:,6:len(featlist[z])+6].values #pairnw kai tis upoloipes pou einai ta xaraktiristika gia pca
            df = StandardScaler().fit_transform(df2)
