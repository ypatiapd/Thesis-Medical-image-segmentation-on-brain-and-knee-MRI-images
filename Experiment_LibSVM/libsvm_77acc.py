# -*- coding: utf-8 -*-
"""
Created on Sun Jan 15 20:18:06 2023
...
#3pca img11 train img1 test  72 acc    11 8 1 posostosi   
#6 eikones ~75 acc
#IMG 26 75 ACC, CHAR 3 3 2 2 POSOSTOSI 5 9 6

#filenames=['img_11_4pca']
#filenames=['image_2_xyz']#,'image_3_xyz','image_4_xyz','image_5_xyz','image_6_xyz','image_10_xyz','image_11_xyz','image_9_xyz','image_12_xyz']#,'image_norm_16','image_norm_20','image_norm_26','image_norm_38','image_norm_39']
#filenames=['image1lbps.txt']#,'image_norm_2','image_norm_3','image_norm_4','image_norm_5','image_norm_6','image_norm_9','image_norm_10','image_norm_11','image_norm_12']

#filenames=['image_norm_6','image_norm_7','image_norm_9','image_norm_11','image_norm_13','image_norm_20','image_norm_26','image_norm_34','image_norm_38','image_norm_39']

#filenames=['image_norm_1']#,'image_norm_20','image_norm_13','image_norm_11','image_norm_9']#,'image_norm_6','image_norm_38','image_norm_39']


#4,6,9,13,20,26,34,38,39,16 label1
#6,7,9,11,13,16,20,26,38,39 label2
# label2-3, 26,20,13,11,39,6,38,9
filenames=['stripped38']#,'img_20_4pca','img_13_4pca','img_11_4pca','img_39_4pca','img_06_4pca']#,'img_38_4pca']#,'img_09_4pca']
#filenames=['image_11_all','image_13_all','image_20_all','image_26_all','image_39_all']
#filenames=['image_6_xyz','image_11_xyz','image_9_xyz','image_13_xyz','image_7_xyz']

#filenames=['image_2_xyz','image_3_xyz','image_4_xyz','image_5_xyz','image_6_xyz','image_10_xyz','image_11_xyz','image_12_xyz','image_13_xyz','image_14_xyz']
#filenames=['image_norm_1','image_norm_2','image_norm_3','image_norm_4','image_norm_5','image_norm_6','image_norm_7','image_norm_9','image_norm_10','image_norm_11']
#,'image_norm_12','image_norm_13','image_norm_15','image_norm_20','image_norm_26','image_norm_34','image_norm_38','image_norm_39'
labeled_x= list()
labeled_y=list()
...
ltrain_size=10000
bootsample_size=round(63*(ltrain_size)/100)
utrain_size = 100000
test_size = 20000
...
ss = np.random.choice(len(Y1)-25, size=len(Y1)-25, replace=False)

'''for i in range(0,ltrain_size):  
    labeled_y.append(copy.copy(Y1[ss[i]]))
    labeled_x.append(copy.copy(X1[ss[i]]))'''
...
for i in classes:
    ...
    for h in range(0,B):  #train B weak annotators on labeled data and predict on unlabeled data
        ...
        sample = np.random.choice(ltrain_size, size=bootsample_size, replace=False) # bootstrap sample for the current weak anotator
        if h==330:
            for j in range(0,len(sample)):
                sample_x.append(copy.copy(labeled_x[sample[j]]))
                sample_y.append(copy.copy(y[sample[j]]))
        else:
            for j in range(0,len(sample)):
                
                temp=dict()
                temp[1]=labeled_x[sample[j]][1]
                #temp[2]=labeled_x[sample[j]][2]
                temp[3]=labeled_x[sample[j]][3]
                temp[4]=labeled_x[sample[j]][4]
                temp[5]=labeled_x[sample[j]][5]            
                
                temp[6]=labeled_x[sample[j]][6]
                #temp[7]=labeled_x[sample[j]][7]
                
                #temp[8]=labeled_x[sample[j]][8]
                #temp[9]=labeled_x[sample[j]][9]    
                #temp[10]=labeled_x[sample[j]][10]
                #temp[11]=labeled_x[sample[j]][11]
                
                #temp[12]=labeled_x[sample[j]][12]
                temp[13]=labeled_x[sample[j]][13]        
                temp[14]=labeled_x[sample[j]][14]
                temp[15]=labeled_x[sample[j]][15]
                
                temp[16]=labeled_x[sample[j]][16]
                temp[17]=labeled_x[sample[j]][17]           
                temp[18]=labeled_x[sample[j]][18]
                temp[19]=labeled_x[sample[j]][19]
            
                temp[20]=labeled_x[sample[j]][20]
                temp[21]=labeled_x[sample[j]][21]
        ...
        #kaloutsika combs
        #param = svm_parameter('-t 2 -g 0.001 -c 10')
        prob = svm_problem(sample_y, sample_x)  #solve simple SVM for each weak anotator
        
        '''if i==1:
            param = svm_parameter('-t 2 -g 0.01 -c 4 -w1 5 -w-1 1')
        else:
            param = svm_parameter('-t 2 -g 0.01 -c 4')'''
        param = svm_parameter('-t 2 -g 0.01 -c 4')
        #prob = svm_problem(sample_y, sample_x)  #solve simple SVM for each weak anotator
    ...
    for k in range (0,iterations):  # k number of iterations , hopefully convergence
        ...
        for j in range(0,B): 
            ...
            #param = parameter('-s 2 -C -B 1')  # na mhn pesei katw apo 10 to -c, palia 100
            
            #c_params = train(prob, param)
            #string= '-s 2 -c '+format(c_params[0])+' -B 1'
            #print('best_c ='+format(c_params[0]))
            #param= parameter(string)
            param= parameter('-s 2 -c 1')
            '''if i==1:
                param= parameter('-s 2 -c 0.5 -B 1')
            if i==2:
                param= parameter('-s 2 -c 8 -B 1')
            if i==3:
                param= parameter('-s 2 -c 0.5 -B 1')'''
            m2=train(prob, param)
