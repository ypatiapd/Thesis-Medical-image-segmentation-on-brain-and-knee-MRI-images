...
if os.path.isdir(write_path):
    ...
    os.mkdir(write_path)
'''
read_path ='C:/Users/ypatia/diplomatiki/hist_imgs'
write_path = 'C:/Users/ypatia/diplomatiki/stand_imgs'
...
'''
#Bias-correction and curvature flow filtering
pool = Pool(4)
pool.map(partial(BC_CF, write_dir = write_path, downsample_factor = downsample_factor, n_iter = n_iter, time_step = time_step), subject_read_dirs)


#Median filtering
pool = Pool(4)
pool.map(partial(median, write_dir = write_path), subject_read_dirs)
'''
