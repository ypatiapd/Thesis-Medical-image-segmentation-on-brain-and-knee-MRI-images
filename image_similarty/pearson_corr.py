# -*- coding: utf-8 -*-
"""
Created on Mon Mar 13 13:07:35 2023
...
num_voxels = np.prod(image1_array.shape[:3])
num_timepoints = image1_array.shape[3]
image1_array = np.reshape(image1_array, (num_voxels, num_timepoints))
...
for ite  in imgs:
    ...
    num_voxels = np.prod(image2_array.shape[:3])
    num_timepoints = image2_array.shape[3]
    image2_array = np.reshape(image2_array, (num_voxels, num_timepoints))

    # Compute the Pearson correlation coefficient between the two 3D arrays
    corr_coef = np.corrcoef(image1_array, image2_array)[0, 1]
    # Compute the mutual information score between the two images
    
    dists.append(corr_coef)
    dists.append(ite)
    images.append(dists)
