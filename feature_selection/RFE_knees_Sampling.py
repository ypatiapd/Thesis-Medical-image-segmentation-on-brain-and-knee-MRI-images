...
imageName = 'C:/Users/jaime/Desktop/Registered_Normalized/normi'+str(iid[0])+'.hdr'
#imageName = 'C:/Users/jaime/Desktop/all_kl0_images_masks/knee'+str(iid[0])+'.hdr'
#imageName = 'C:/Users/jaime/Desktop/all_kl0_median_images/median_knee'+str(iid[0])+'.hdr'
#imageName = 'C:/Users/jaime/Desktop/all_kl0_histogrammed_images/hist_knee'+str(iid[0])+'.hdr'
#imageName = 'C:/Users/jaime/Desktop/all_kl0_standardized_images/standard_knee'+str(iid[0])+'.hdr'
...
del maskArr,bboxs,bbox,bbox2,bbox3,bbox4
# TURN OFF THE PLOTTER FOR RANDOM FOREST
mask_arr = sitk.GetArrayFromImage(mask)
'''
'''
for i in range(1,len(mask_arr[0][0])):  # edw to i metraei panw sta slices ths prwths 8eshs tou ROI 
    ...
    ax2.imshow(image_arr[:,:,i-1], cmap='gray')
...
indices = random.sample(range(len(indices1[0])), 500000) #1041639
#indices.sort()
...
'''
indices = random.sample(range(len(indices2[0])), 6000)
#indices.sort()
...
indices = random.sample(range(len(indices3[0])), 150000) #218876
#indices.sort()
...
'''
totindex = np.concatenate((array1x,array2x,array3x,indices4[0]),axis=0)
totindey = np.concatenate((array1y,array2y,array3y,indices4[1]),axis=0)
totindez = np.concatenate((array1z,array2z,array3z,indices4[2]),axis=0)
'''

totindex = np.concatenate((array1x,indices2[0],array3x,indices4[0]),axis=0)
totindey = np.concatenate((array1y,indices2[1],array3y,indices4[1]),axis=0)
totindez = np.concatenate((array1z,indices2[2],array3z,indices4[2]),axis=0)
...
for i in range(0,len(totindex)):
    ...
    if mask_arr[totindex[i]][totindey[i]][totindez[i]] == 3:
        count3 +=1 
    if mask_arr[totindex[i]][totindey[i]][totindez[i]] == 4:
        count4 +=1 
...
del totindex,totindey,totindez,array3x,array3y,array3z
del array1x,array1y,array1z,indices,indices1
del indices2,indices3,indices4
del totindexx
...
del L1,L2,L3,L4,b1,b2,b3,b4,c1,c2,c3,c4,lbp_arr1,lbp_arr21,lbp_arr31,lbp_arr41,level1,level2,level21,level22,level31,level32,level41,level42     
...
del maskArr,bboxs,bbox,bbox2,bbox3,bbox4,image_arr
...
for z in range(0,len(featlist)):
    ...
    del label1,label2,label3,label4,Adder,Bidder,res,res2
...
del set_of_lists, img_views, mask_arr, mask_view
