...
#imageName1='C:/Users/ypatia/diplomatiki/registered_imgs/registered'+ite+'.hdr'
#imageName2='C:/Users/ypatia/diplomatiki/denoised/denoised'+ite+'.hdr'
...
######gia plagia peristrameni opsi egefalwn########
'''z_slice1 = image_arr1[100,:,:]

# Rotate the z-slice to the left
rotated_slice1 = np.transpose(z_slice1)[::-1]
rotated_slice1 = np.transpose(rotated_slice1)[::-1]

# Replace the z-slice in the original image with the rotated slice
image_arr1[100,:,:] = rotated_slice1

z_slice2 = image_arr2[100,:,:]

# Rotate the z-slice to the left
rotated_slice2 = np.transpose(z_slice2)[::-1]
rotated_slice2 = np.transpose(rotated_slice2)[::-1]

# Replace the z-slice in the original image with the rotated slice
image_arr2[100,:,:] = rotated_slice2'''
######gia plagia peristrameni opsi egefalwn########


######gia katopsi egefalwn########
image_arr1 = np.swapaxes(image_arr1,0,1)
image_arr1 = np.swapaxes(image_arr1,1,2)

image_arr2 = np.swapaxes(image_arr2,0,1)
image_arr2 = np.swapaxes(image_arr2,1,2)
...
for i in ite:
    ...
    dim1, dim2, dim3 = image_arr1.shape
    
    # Create a figure with two subplots
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(6, 6))

    # Plot the mask in the first subplot
    ax1.imshow(image_arr1[i,:,:], cmap='gray')
    ax1.set_title('Original MRI')
    ax1.axis('off')
    # Plot the image in the second subplot
    ax2.imshow(image_arr2[i,:,:], cmap='gray')
    ax2.set_title('Scull Stripped MRI')
    ax2.axis('off')
